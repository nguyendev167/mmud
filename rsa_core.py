...
class RSACore:
    ...
    # ==========================
    # Miller Rabin (Thuật toán snt tối ưu cho isprime)
    # ==========================
    @staticmethod
    def is_prime(n, k=20):
        if n < 2:
    ...
    def extended_gcd(a, b):
        if b == 0:
            return a, 1, 0
        gcd, x1, y1 = RSACore.extended_gcd(b,a % b)
        x = y1
        y = x1 - (a // b) * y1
        return gcd, x, y
    ...
    def mod_inverse(e, phi):
        g, x, _ = RSACore.extended_gcd(e, phi)
        if g != 1:
    ...
    def encrypt(message_bytes, public_key):
        ...
        if m >= n:
            raise ValueError("Message quá lớn")
        c = RSACore.mod_exp(m, e, n)
        return c
    # ==========================
    ...
    def decrypt(cipher_int, private_key):
        d, n = private_key
        m = RSACore.mod_exp(cipher_int, d, n)
    ...
    def encrypt_int(message_int, e, n):
        if message_int >= n:
            raise ValueError("Message qua lon")
        return RSACore.mod_exp(message_int, e, n)
    ...
    def decrypt_int(cipher_int, d, n):
        return RSACore.mod_exp(cipher_int, d, n)
